...
class MCU_stepper:
    ...
    def calc_position_from_coord(self, coord):
        ...
        return ffi_lib.itersolve_calc_position_from_coord(
            ...
            coord[3] if len(coord) > 3 else 0.,
            coord[4] if len(coord) > 4 else 0.,
            coord[5] if len(coord) > 5 else 0.)
...
class GenericPrinterRail:
    def __init__(self, config, need_position_minmax=True,
        ...
        self.homing_endstop_offset = config.getfloat(
            'homing_endstop_offset', 0.)
